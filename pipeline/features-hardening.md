...
| `award_terms_description`, `award_calculation_expression` | `string` | `AwardingTerms/AwardingCriterion/Description`, `.../CalculationExpression` (parent level, NOT Subordinate) | **Corpus finding (2026-07-30):** the null `award_criterion_kind` lots are NOT blocks-missing-codes (only 7 such lots exist). 773 lots have no `SubordinateAwardingCriterion` at all; 628 of those have no AwardingTerms either, but ~145 carry classifiable text at the parent level. These two columns preserve that text. |
| `award_criterion_kind` fallback | — | the two parent-level texts above | Classify only when unambiguous, flag `criterion_kind_from_text`: **meat** for `der preis ist nicht das einzige zuschlagskriterium` (57×) and the eForms English MEAT boilerplate `most economically advantageous tender` (24×); **price-only** for a strict whitelist (`zusch(l)agskriterium:? preis`, `alleiniges zuschlagskriterium = preis`, `^(100 ?% )?preis( 100 ?%)?\.?$`, ~30×). Everything else (criteria "in den Vergabeunterlagen", "-", capability texts) stays null. Realistic recovery: ~110 of ~780 unknown lots — NOT the "27% → single digits" originally claimed here; most unknowns publish nothing at all. |
