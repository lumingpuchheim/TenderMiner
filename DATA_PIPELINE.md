...
Stage 1: TED notices  ──►  Stage 2: GAEB documents  ──►  Stage 3: feature extraction
        (metadata + text)      (best-effort, quantities)      (one feature record per notice)
```
...
3. Download the document package; keep only GAEB files; store them under
   `data/gaeb/<publication-number>/`.
4. **Log one outcome per notice** (this log is itself data — it measures link rot):
...
5. Retries are allowed while a tender is live (e.g. daily until deadline); after the
...
1. **GAEB file** (when Stage 2 yielded one): parse positions → structured quantities
   (m², m³, m, t, Stück) with what each quantity refers to. This is registry-grade
   input — no LLM needed, it is a defined format.
...
Append-only JSONL, one record per notice, keyed by `publication-number` +
`procedure-identifier` — never overwritten, new records only (consistent with the
append-only ingestion principle):

```jsonc
// data/features.jsonl — one line per notice
{
  "publication-number": "518801-2026",
