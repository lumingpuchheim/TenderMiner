...
| `data/admin_cores.json` (0.8 MB) | every winner's trade, for the operator's page (doc/ADMIN.md §4) | the same kind of thing one level up: a pure function of the store and the evidence rules, rewritten wholesale, read once per process. Carries the store mtimes and a rules-plus-vocabulary stamp inside it, so a stale one is recomputed rather than served; delete it and the next page rebuilds it in ~30 s |
