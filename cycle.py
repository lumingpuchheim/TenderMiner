"""TenderMining cycle — the update: download -> grade -> learn -> predict ->
report. Runs any day, as often as you like, and mails nobody.

Concept: ONLINE_LEARNING.md. Model logic: single_bidder.py (v1 notice-only
features; no buyer-derived features by design decision). The sending is a
separate command, `deliver.py`, which reads what this wrote (RUNBOOK 1).

    python cycle.py run --last 7d              # the update
    python cycle.py run --last 6m              # first backfill
    python cycle.py run --last 7d --skip-download   # offline: reuse the store
...
module named for it — `grading`, `training`, `predicting`, `drift`, `report`,
`housekeeping`, over `util` — and `run()` below is that list, one named
function per step, in order. It re-exports none of them: a caller that wants
`flag_stats` imports `grading`, not this. The download stayed here because it
is two subprocess calls and a window subtraction — orchestration, not a
step's logic.

Until 2026-08-18 this file was `loop.py` and its last step rendered and
mailed every customer from the scores still in memory. That is now
`deliver.py`; nothing here writes a delivery row or sends anything.
"""
...
def load_store(paths):
    """The three frames every later step reads: tenders with roles, awards,
    and the labeled training frame."""
    ...
    return tenders, roles, awards, data, aw


# ---------------------------------------------------------------- 3. embed

def embed_sidecar(paths, tenders):
...
# ------------------------------------------------- 4. grade, learn, predict

def learn_and_score(paths, args, checkpoint, tenders, roles, data, aw):
    ...
    to the monitors, the report and the simulation. With none it is the
    single implicit arm — exactly the cycle as it always was.

    Returns everything the later steps read: the plan, the new grades, the
    track record, the delivering arm's model id and gate, and its scores."""
    ...
    return plan, new_grades, record, model_id, gate, rows, scores_now, scored


# ---------------------------------------------------- 5. monitors and report

def monitor_and_report(paths, args, tenders, aw, plan, new_grades, record,
                       model_id, gate, rows, scores_now, scored):
    """Drift monitors, the trial's verdict line, the shadow judgments, the
    knob proposals and the gate guard — then the operator report that
    carries them all. Returns whether the gate guard passed, which
    `deliver.py` reads again on its own; here it only goes in the report."""
    ...
    # the gate guard, which stops DELIVERY only (deliver.py checks it again) —
    # this cycle's grading, training and predictions are written regardless.
...
def simulate(paths, args, tenders, aw, scored):
    import simulation
...
# -------------------------------------------- 7. dashboard, site, caches

def publish(paths, args):
    """The operator's and the public's derived views. Every one of these is
    non-fatal by the same rule: a week-stale page is acceptable, a failed
    cycle is not."""
    try:
    ...
    #
    # Nothing uploads here: the edge serves `<data>/public/current` directly,
    # and `release` swaps it all-or-nothing. A page-TEMPLATE change does not
    # wait for Monday either — docker/deploy.sh runs the same build after
    # every deploy.
    ...
    # and not half a minute of tokenising.
    try:
        import admin
...
def run(paths, args):
    """The steps above, in order. Reads as the table in RUNBOOK 1."""
    ...
    # is the one every delivery row of this week will carry.
    ...
    tenders, roles, awards, data, aw = load_store(paths)
    embed_sidecar(paths, tenders)
    (plan, new_grades, record, model_id, gate,
     rows, scores_now, scored) = learn_and_score(paths, args, checkpoint,
                                                 tenders, roles, data, aw)
    monitor_and_report(paths, args, tenders, aw, plan, new_grades, record,
                       model_id, gate, rows, scores_now, scored)
    simulate(paths, args, tenders, aw, scored)
    publish(paths, args)
...
def cmd_run(args):
    ...
    # replay someone started at 06:55 is over in minutes, while a skipped
    # Monday is a week with no fresh predictions. The wait is bounded —
    # heavy_lock raises rather than hanging, and property 3 in that module
    # is why.
    with heavy_lock.held(paths.data, 'the cycle', wait=3600):
        run(paths, args)
...
def add_common_args(p):
    """The arguments the cycle AND the delivery share — the ones that shape
    a customer's view of a prediction row — so a report rendered by
    deliver.py agrees with the cycle that wrote the rows. One definition,
    two parsers (deliver.py imports this)."""
    p.add_argument('--track-window', default='12w', dest='track_window',
                   help='track-record reporting window (default 12w)')
    p.add_argument('--top-slice', type=float, default=0.2, dest='top_slice',
                   help='share of the ranking counted as "our picks" in rank-based metrics (default 0.2)')
    p.add_argument('--tier-high', type=float, default=0.10, dest='tier_high',
                   help='share of the weekly ranking tiered HIGH (default 0.10)')
    p.add_argument('--tier-medium', type=float, default=0.20, dest='tier_medium',
                   help='share of the weekly ranking tiered MEDIUM, after HIGH (default 0.20)')
    p.add_argument('--data-dir', default=config.data_root(), dest='data_dir')
    p.add_argument('--models-dir', default=config.models_root(), dest='models_dir')
...
def main():
    ...
    run_.add_argument('--min-trade-grades', type=int, default=25, dest='min_trade_grades',
                      help='minimum graded lots per trade before its track record is reported')
    run_.add_argument('--min-flag-grades', type=int, default=30, dest='min_flag_grades',
                      help='graded lots below which the precision/recall section says so '
                           'out loud before quoting itself (default 30)')
    run_.add_argument('--val-window', default='8w', dest='val_window',
                      help='validation window for the promotion gate (default 8w)')
    run_.add_argument('--min-val-lots', type=int, default=30, dest='min_val_lots',
                      help='minimum lots in the validation window to run the gate')
    run_.add_argument('--min-shuffle-positives', type=int, default=20, dest='min_shuffle_positives',
                      help='minimum positive val lots for the shuffled-label check to be meaningful')
    run_.add_argument('--promote-epsilon', type=float, default=0.005, dest='promote_epsilon',
                      help='candidate may trail the champion by this much and still promote')
    run_.add_argument('--drift-window', default='4w', dest='drift_window',
                      help='"recent" window for the drift monitors (default 4w)')
    run_.add_argument('--drift-min-lots', type=int, default=30, dest='drift_min_lots',
                      help='minimum rows on each side before a drift monitor speaks (default 30)')
    run_.add_argument('--missing-jump', type=float, default=0.15, dest='missing_jump',
                      help='null-rate change that counts as missingness drift (default 0.15)')
    run_.add_argument('--psi-warn', type=float, default=0.25, dest='psi_warn',
                      help='PSI above which the score distribution has drifted (default 0.25)')
    run_.add_argument('--iterations', type=int, default=None,
                      help='CatBoost iterations override (testing)')
    run_.add_argument('--report-top', type=int, default=30, dest='report_top',
                      help='open lots listed in the report')
    run_.add_argument('--sim-max-picks', type=int, default=5, dest='sim_max_picks',
                      help='picks per simulated winner company per cycle (default 5)')
    run_.add_argument('--sim-min-deadline-days', type=int, default=14,
                      dest='sim_min_deadline_days',
                      help='deadline floor for simulated picks, like the product default')
    run_.add_argument('--skip-download', action='store_true', dest='skip_download',
                      help='reuse the existing store (offline run)')
    run_.add_argument('--skip-trade-pages', action='store_true',
